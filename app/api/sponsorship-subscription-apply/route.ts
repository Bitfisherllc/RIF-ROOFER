...
export async function POST(request: NextRequest) {
  try {
    ...
    const notificationHtml = getRooferFullApplyNotificationEmailHtml(body, 'Sponsored Listing / Subscription');
    ...
    const { data, error } = await resend.emails.send({
      from: fromEmail,
      to: [targetEmail],
      subject: `Sponsored results signup: ${body.companyName}`,
    ...
    if (isVerificationError) {
      ...
      const fallback = await resend.emails.send({
        from: fromEmail,
        to: [verifiedEmail],
        subject: `[FORWARD] Sponsored results signup: ${body.companyName}`,
      ...
      if (fallback.error) {
        ...
        return NextResponse.json(
          { error: 'Failed to send subscription request. Please try again or contact us directly.' },
          { status: 500 }
        );
      }
      return NextResponse.json(
        { success: true, message: 'Subscription request submitted successfully.' },
        { status: 200 }
      );
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to send subscription request. Please try again.' },
        { status: 500 }
      );
    }

    return NextResponse.json(
      { success: true, message: 'Subscription request submitted successfully.', data },
      { status: 200 }
    );
  } catch (err) {
    console.error('Sponsored results apply error:', err);
    return NextResponse.json(
