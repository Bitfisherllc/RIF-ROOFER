...
import { getAllTrainingPaths } from '@/app/training/data/training-paths';
import type { TrainingEvent, TrainingEventCategory } from '@/app/training/data/events';
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-16">
            {trainingPaths.map((path) => {
              ...
              return (
                <Link
                  key={path.id}
                  href={`/training-schedule/${path.id}`}
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {filteredEvents.map((event) => (
                <Link
                  key={event.id}
                  href={`/training-schedule/events/${event.id}`}
